 ...
 */
function valueEquals(rowValue: unknown, filterValue: string | number | null): boolean {
  // null emulates `.is(col, null)`, NOT `.eq(col, null)` — PostgREST cannot
  // express the latter, and every caller passing null means IS NULL. Needed for
  // owner scoping: the shared master card is the rows with owner_id IS NULL.
  if (filterValue === null) return rowValue == null;
 ...
 *
 * OWNER SCOPING (trial-role isolation, supabase/migration-trial-role.sql): the
 * cache holds EVERY owner's rows in one process-wide table, so callers must
 * pass `owner_id` like any other filter — null for the shared master card, a
 * user id for that trial's private clone. Omitting it does not merely widen a
