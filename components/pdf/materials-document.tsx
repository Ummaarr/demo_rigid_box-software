...
// the right, in a bordered card. Branding (navy/gold, logo, tagline) is kept
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 15, fontFamily: "Helvetica-Bold", color: NAVY, letterSpacing: 1, marginTop: 12, marginBottom: 5 },
  ...
  componentLabel: { fontSize: 10, fontFamily: "Helvetica-Bold", color: NAVY, marginTop: 8, marginBottom: 2 },
  ...
  blockHeading: { fontSize: 9, fontFamily: "Helvetica-Bold", color: NAVY, marginBottom: 3 },
  ...
  totalLine: { fontSize: 9, fontFamily: "Helvetica-Bold", color: NAVY, marginTop: 4 },
  ...
  finTitle: { fontSize: 8.5, fontFamily: "Helvetica-Bold", color: NAVY, marginTop: 3 },
  // Lists
  ...
  return (
    <View style={{ marginBottom: 3 }}>
      <Svg width={DIAG_W} height={DIAG_H} viewBox={`0 0 ${VB.W} ${VB.H}`}>
        <Rect x={VB.ML} y={VB.MT} width={sW} height={sH} fill="white" stroke={NAVY} strokeWidth={1.5} />
        {d.rects.map((r, i) => (
          <Rect
            key={i}
          ...
          return (
            <View key={i} style={{ width: "33%", marginBottom: 4, alignItems: "center" }} wrap={false}>
              <Svg width={DIAG_W * 0.6} height={DIAG_H * 0.6} viewBox={`0 0 ${VB.W} ${VB.H}`}>
                <Rect x={VB.ML} y={VB.MT} width={k.blankW_in * scale} height={k.blankH_in * scale} fill="white" stroke={NAVY} strokeWidth={1.5} />
                {k.creases.map((c, j) => (
                  <Line
                    key={j}
                    x1={VB.ML + c.x1 * scale}
